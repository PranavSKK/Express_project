...
app.get("/filter", (req,res)=>{
  ...
  if (result.length > 0) {
    ...
    res.json("Jokes with type not found");
  }
});
...
app.post("/jokes", (req,res)=>{
  const body = req.body
  if (!body.text){
    res.json("Body Text Params missing")
  }
  const newJoke = {
    id: jokes.length + 1,
    jokeText: body.text,
...
app.delete("/jokes/all", (req,res)=>{
  ...
  if (masterKey === key){
    jokes = []
    res.json("Successfully deleted jokes")
  } else {
    res.json("Key not matching")
  };
});
